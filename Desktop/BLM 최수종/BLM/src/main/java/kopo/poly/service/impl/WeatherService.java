...
public class WeatherService implements IWeatherService {
    ...
    public WeatherDTO getWeather(WeatherDTO pDTO) throws Exception {
        ...
        // 일별 날씨 조회(OpenAPI가 현재 날짜 기준으로 최대 7일까지 제공) // json viewer에서 []로 표시되는 JSONARRAY는 List<Map<String,Object>> 타입으로 받음
        ...
        for (Map<String, Object> dailyMap : dailyList) {
            ...
            for (Map<String, Object> weatherMap : weatherList) {
                icon = String.valueOf(weatherMap.get("icon"));
            }
            ...
            pList.add(wdDTO); // 일별 날씨 정보를 List에 추가하기
                              // for문밖에 선언된 pList에 담아줬으므로 for문이 끝나고도 값을 계속 쓸 수 있음.
        ...
        log.info(this.getClass().getName() + ".getWeather 끝!");
